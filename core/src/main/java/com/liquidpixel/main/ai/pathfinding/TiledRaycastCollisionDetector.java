package com.liquidpixel.main.ai.pathfinding;

import com.badlogic.gdx.ai.utils.Collision;
import com.badlogic.gdx.ai.utils.Ray;
import com.badlogic.gdx.ai.utils.RaycastCollisionDetector;
import com.badlogic.gdx.math.GridPoint2;
import com.badlogic.gdx.math.Vector2;
import com.liquidpixel.main.ai.pathfinding.TiledGraph;
import com.liquidpixel.main.ai.pathfinding.TiledNode;
import com.liquidpixel.main.model.sprite.NodeType;

public class TiledRaycastCollisionDetector<N extends TiledNode<N>> implements RaycastCollisionDetector<Vector2> {
    TiledGraph<N> worldMap;

    public TiledRaycastCollisionDetector(TiledGraph<N> worldMap) {
        this.worldMap = worldMap;
    }

    @Override
    public boolean collides(Ray<Vector2> ray) {
        int x0 = (int)ray.start.x;
        int y0 = (int)ray.start.y;
        int x1 = (int)ray.end.x;
        int y1 = (int)ray.end.y;

        int tmp;
        boolean steep = Math.abs(y1 - y0) > Math.abs(x1 - x0);
        if (steep) {
            tmp = x0; x0 = y0; y0 = tmp;
            tmp = x1; x1 = y1; y1 = tmp;
        }
        if (x0 > x1) {
            tmp = x0; x0 = x1; x1 = tmp;
            tmp = y0; y0 = y1; y1 = tmp;
        }

        int deltax = x1 - x0;
        int deltay = Math.abs(y1 - y0);
        int error = 0;
        int y = y0;
        int ystep = (y0 < y1 ? 1 : -1);
        int lastX = x0;
        int lastY = y0;

        for (int x = x0; x <= x1; x++) {
            N tile = steep ? worldMap.getNode(new GridPoint2(y, x)) : worldMap.getNode(new GridPoint2(x, y));

            // Check current tile
            if (tile.getType() != NodeType.TILE_FLOOR) return true;

            // Check for diagonal movement
            if (x != lastX && y != lastY) {
                // Check both corner tiles when moving diagonally
                N corner1 = steep ? worldMap.getNode(new GridPoint2(lastY, x)) : worldMap.getNode(new GridPoint2(x, lastY));
                N corner2 = steep ? worldMap.getNode(new GridPoint2(y, lastX)) : worldMap.getNode(new GridPoint2(lastX, y));

                if (corner1.getType() != NodeType.TILE_FLOOR || corner2.getType() != NodeType.TILE_FLOOR) {
                    return true;
                }
            }

            lastX = x;
            lastY = y;

            error += deltay;
            if (error + error >= deltax) {
                y += ystep;
                error -= deltax;
            }
        }

        return false;
    }

    @Override
    public boolean findCollision(Collision<Vector2> outputCollision, Ray<Vector2> inputRay) {
        throw new UnsupportedOperationException();
    }
}
